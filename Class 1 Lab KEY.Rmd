...
```{r}
C1survey$R_exp <- as.factor(C1survey$R_exp)

C1survey.cleaned <- C1survey %>%
select(R_exp, coding_comfort ) %>% 
  zap_labels() %>% 
  
  
  
mutate(coding_comfort= recode_factor(.x = coding_comfort,
                                  `1` = 'Not comfortable',
...
# Examination of the experience in R variable
summary(object = C1survey.cleaned$R_exp)

#  Descriptive statistics

C1survey.cleaned$R_exp <- as.numeric(as.character(C1survey.cleaned$R_exp))
C1survey.cleaned %>%
  drop_na(R_exp) %>%
  summarize(mean.exp = mean(x = R_exp),
            sd.exp = sd(x = R_exp),
            var.exp = var(x = R_exp),
            med.exp = median(x = R_exp),
            iqr.exp = IQR(x = R_exp),
            mode.exp = names(x = sort(x = table(R_exp),
                                       decreasing = TRUE))[1])  
```
...
```{r}
# Spearman's rho correlation test between R_exp and coding_comfort
correlation_test <- cor.test(as.numeric(C1survey.cleaned$R_exp), 
                             as.numeric(C1survey.cleaned$coding_comfort), 
                             method = "spearman")
